...
func NewIndexer(url, apiKey, indexerName string) *Indexer {
	...
	inner := inner_meilisearch.NewIndexer(url, apiKey, indexerName, codeIndexerLatestVersion, settings)
	indexer := &Indexer{
		inner:   inner,
		Indexer: inner,
	}
...
// Delete deletes indexes by repoID
func (b *Indexer) Delete(ctx context.Context, repoID int64) error {
	_, err := b.inner.Client.Index(b.inner.VersionedIndexName()).DeleteDocument(strconv.FormatInt(repoID, 10))
	return err
}
	...
	return []elastic.BulkableRequest{
		elastic.NewBulkIndexRequest().
			...
			Doc(map[string]any{
				...
				"updated_unix": timeutil.TimeStampNow(),
			}),
	}, nil
}
...
func convertResult(searchResult *elastic.SearchResult, kw string, pageSize int) (int64, []*internal.SearchResult, []*internal.SearchResultLanguages, error) {
	...
	for _, hit := range searchResult.Hits.Hits {
		...
		hits = append(hits, &internal.SearchResult{
			...
			UpdatedUnix: timeutil.TimeStamp(res["updated_unix"].(float64)),
			Language:    language,
